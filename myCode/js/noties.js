...
//--- Декоратор на функцию ------------------------------------------
    let f = (time, ...arg) => {
        console.log(`функция f c таймаутом в ${time} и следующими аргументами: ${arg} `);
    }

    function dec(func) {                    //передали функцию
        return (...arg) => {                //передали аргументы
            let [time] = [...arg];          //деструктизируем
            arg.shift(0);                   //удалим первый элемень массива
            return setTimeout(() => {       //оборачиваем
                return func(time, arg);     //вызываем
            }, time);
        }
    }

    let wrTime = dec(f);                    //декарируем
    wrTime(1000, 'str1', 'str2'); //= произвольная фукнция + c таймаутом в 1000 и следующими аргументами: str1,str2 
